...
type SymbolFun = Box<dyn for<'a> Fn(&'a mut Counts, &'a mut u16, &'a mut u16) -> Option<&'a mut u16>>;
            ...
            .filter_map(|(&range_start, name)| {
                match name.as_str() {
                    "net_rx_action" => Option::<SymbolFun>::Some(Box::new(
                        |cnt, _, _| Some(&mut cnt.net_rx_action)
                    )),
                    "__napi_poll" => Option::<SymbolFun>::Some(Box::new(
                        |cnt, _, _| Some(&mut cnt.__napi_poll)
                    )),
                    "netif_receive_skb" | "netif_receive_skb_core" | "netif_receive_skb_list_internal" | "napi_gro_receive" => Option::<SymbolFun>::Some(Box::new(
                        |cnt, in_nf_hook, _| {
                    ...
                    "do_xdp_generic" => Option::<SymbolFun>::Some(Box::new(
                        |cnt, _, _| Some(&mut cnt.do_xdp_generic)
                    )),
                    "tcf_classify" => Option::<SymbolFun>::Some(Box::new(
                        |cnt, _, _| Some(&mut cnt.tcf_classify)
                    )),
                    ...
                    "nf_hook_slow" => Option::<SymbolFun>::Some(Box::new(
                        |_, in_nf_hook, _| Some(in_nf_hook)
                    )),
                    "ip_rcv" => Option::<SymbolFun>::Some(Box::new(
                        |cnt, in_nf_hook, ip_rcv_finish| {
                            if *ip_rcv_finish == 0 {
                    ...
                    "ip_rcv_finish" | "ip6_rcv_finish" => Option::<SymbolFun>::Some(Box::new(
                        |_, _, ip_rcv_finish| Some(ip_rcv_finish)
                    )),
    ...
    pub unsafe fn acc_trace(&mut self, ksyms: &KSyms, trace_ptr: *const u64, max_frames: usize) {
        ...
        for frame_idx in 0..max_frames {
            ...
            if let Some((_, KSymsVal { range_end, fun })) = ksyms
                ...
                .next_back() {
                    if ip < *range_end {
                        if let Some(cnt) = fun(&mut c, &mut in_nf_hook, &mut ip_rcv_finish) {
                            *cnt = 1;
                        }
                    }
                }
